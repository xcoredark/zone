...
export Server="raw.githubusercontent.com/xcoredark/zone/main/limited"
#!/bin/bash
clear
echo -e "\e[36m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m"
...
echo -e "${OKEY} Starting Generating Certificate"
        ...
        wget -q -O /root/.acme.sh/acme.sh "https://${Server}/acme.sh"
        chmod +x /root/.acme.sh/acme.sh
        sudo /root/.acme.sh/acme.sh --issue -d $domain --standalone -k ec-256
        sudo /root/.acme.sh/acme.sh --installcert -d $domain --fullchainpath /etc/xray/xray.crt --keypath /etc/xray/xray.key --ecc
        sleep 2
# nginx renew ssl
echo -n '#!/bin/bash
...
wget -q -O core "https://${Server}/core.sh"
wget -q -O cek-usage "https://${Server}/cek-usage.sh"
wget -q -O add-host "https://${Server}/add-host.sh"
wget -q -O cf "https://${Server}/cf.sh"
wget -q -O cert "https://${Server}/cert.sh"
...
echo "   Script Xray Only Sukses Terinstall   "  | tee -a log-install.txt
echo "--------------------------------------" | tee -a log-install.txt
echo "======================================" | tee -a log-install.txt
echo "   ------------ Service & Port--------" | tee -a log-install.txt
echo "   - All Vmess               : 80, 443" | tee -a log-install.txt
echo "   - All Showdowsock         : 80, 443" | tee -a log-install.txt
echo "   - All Trojan              : 80, 443" | tee -a log-install.txt
echo "   - All Vless               : 80, 443" | tee -a log-install.txt
echo ""
echo "" | tee -a log-install.txt
echo "--------------------------------------" | tee -a log-install.txt
echo "===Script Installer By Kang Mardhex===" | tee -a log-install.txt
echo ""
echo " Server Reboot Otomatis"
sleep 5
rm -f setup.sh
reboot
